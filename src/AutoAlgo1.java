...
public class AutoAlgo1 {
	...
	Stack<Pair<Point, Double>> stackWay;
	...
	public void updateVisited() {
		Point dronePoint = drone.getOpticalSensorLocation();
		Point fromPoint = new Point(dronePoint.x + droneStartingPoint.x,dronePoint.y + droneStartingPoint.y);
		if(SimulationWindow.toogleAI && !SimulationWindow.return_home) {
			stackWay.add(new Pair<>(dronePoint, (this.drone.getGyroRotation() + 180) % 360));
		}
	...
	*/
	...
	public void return_home_ai(int delta_time) {
		if (!SimulationWindow.toogleAI) {
			return;
		}
		Point dronePoint = drone.getOpticalSensorLocation();
		if (Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) < max_distance_between_points) {
			if (points.size() <= 1 && Tools.getDistanceBetweenPoints(getLastPoint(), dronePoint) < max_distance_between_points / 5) {  // we got home!
				speedDown();
			} else {
				removeLastPoint();
			}
...
//		}
		...
		if(!is_risky) {
			...
			if(!try_to_escape) {
				...
				Lidar lidar_up = drone.lidars.get(2);
				double lidar_up_dist = lidar_up.current_distance;
